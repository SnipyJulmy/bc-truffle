...
public final class BcMain {
    ...
    public static void main(String[] args) throws IOException {
        ...
        if (file == null) {
            // @formatter:off
            source = Source.newBuilder(Bc, new InputStreamReader(System.in), "<stdin>").build();
            // @formatter:on
        } else {
            source = Source.newBuilder(Bc, new File(file)).build();
        }
    ...
    private static int executeSource(Source source, InputStream in, PrintStream out, Map<String, String> options) {
        ...
        try {
            context = Context.newBuilder(Bc).in(in).out(out).options(options).build();
